...
function _Recv(args, body, bodyFile)
	-- 参数判断
    if not LuaEx.IsTable(args) or LuaEx.IsNullOrEmpty(args.action) or not LuaEx.IsFunction(_G["S2LogS"][args.action]) then
    	assert(false, "LogServer Action Error : " .. (args.action or "nil"))
    end
    -- 执行
    return _G["S2LogS"][args.action](args, body, bodyFile)
end
